...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.95\textwidth]{assets/system_architecture.png}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.82\textwidth]{assets/telemetry_pipeline.png}
...
Generated artifacts include \texttt{data/circle\_flight\_animation.gif}, while every manuscript figure and Graphviz source file is exported to \texttt{latex/assets/} for direct inclusion in the final report.
...
To study controller tuning, three LQR weight sets were tested: conservative \((Q=\mathrm{diag}(20,8),R=2)\), baseline \((Q=\mathrm{diag}(30,10),R=1)\), and aggressive \((Q=\mathrm{diag}(60,15),R=0.5)\). These are project-defined comparison cases rather than externally prescribed textbook settings. Figure \ref{fig:lqr-weight-sensitivity} summarizes the results.
...
Finally, the supplementary RK4/Euler benchmark was recreated using the project dynamics in order to ensure methodological consistency. The recreated case retains the benchmark settings \(\Delta t_{\mathrm{ref}}=0.05\,\mathrm{s}\), \(\Delta t_{\mathrm{test}}=0.5\,\mathrm{s}\), \(T_{\mathrm{sim}}=100\,\mathrm{s}\), and \(\phi_{\mathrm{cmd}}=15^\circ+10^\circ\sin(2\pi t/20)\). Figure \ref{fig:rk4-vs-euler-project} shows the resulting comparison.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.90\textwidth]{assets/module_dependency.png}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.86\textwidth]{assets/controller_logic.png}
