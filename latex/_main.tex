...
Figure \ref{fig:appendix-part1} complements Table \ref{tab:tracking-metrics} by showing how the cross-track and angular errors decay, how the commanded and realized bank angles align, and how thrust and stage objective remain well behaved during convergence.
...
The timing-distribution figure supplements the mean and percentile values reported in Table \ref{tab:timing-metrics}. It shows not only that the average runtime is small, but also that the distributions are compact and do not exhibit problematic long tails under the current experimental conditions.
...
Figures \ref{fig:appendix-part3} and \ref{fig:estimation-covariance-detail} provide a more detailed view of the estimator behavior. The difference between \(\mathrm{tr}(P^-)\) and \(\mathrm{tr}(P^+)\) reflects uncertainty reduction after each update, while the MAP objective shows how the posterior balances model consistency and measurement agreement over time.
...
Figure \ref{fig:appendix-part2} complements the global overview by showing the dynamic states directly. The nominal trajectory follows the reference geometry cleanly, and the translational, angular, and speed channels all remain smooth and bounded throughout the maneuver.
...
Figure \ref{fig:trajectory-family} adds a geometric interpretation to the scalar sensitivity metrics. It shows visually that all tested cases converge toward their respective circles without pathological behavior, even though the curvature and required bank-angle demand change substantially.
